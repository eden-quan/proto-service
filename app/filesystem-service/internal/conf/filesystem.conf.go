...
// FilesystemServiceConfig 在全局基础配置的前提下增加了自定义配置信息 Hello,
// 并且在此之上，通过 `conf_service` 增加了对基础配置自定义的能力
type FilesystemServiceConfig struct {
	*def.Configuration `conf_service:"filesystem-service"`
	// Server            *def.Server        `conf_path:"/registry/filesystem-service/v1/config"` // TODO: 配置完配置中心后移除注释, 实现自定义服务器监听配置
...
// NewFilesystemServiceConfig 提供新的自定义配置信息
func NewFilesystemServiceConfig(repo config.ConfigureWatcherRepo) (*FilesystemServiceConfig, error) {
	...
	err := repo.LoadAndStart(filesystem)
	return filesystem, err
}

// NewConfigReplacer 允许应用使用自定义配置信息覆盖默认的全局配置
func NewConfigReplacer(filesystem *FilesystemServiceConfig) *def.Configuration {
	filesystem.Configuration.Server = filesystem.Server
	return filesystem.Configuration
}

func init() {
	inject.Injection().Inject(NewFilesystemServiceConfig)
